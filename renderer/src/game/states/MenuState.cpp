...
namespace renderer
{
    ...
    void MenuState::processInput(sf::Event& event, StateManager& stateManager, const sf::RenderWindow& window)
    {
        ...
        {
            std::unique_ptr<IState> levelState = Game::getInstance()->
                                                 getAppConfig().getRendererConfig().getFactoryCollection().
    ...
    void MenuState::draw(sf::RenderWindow& window, StateManager& stateManager)
    {
        if (!_fontLoaded)
        {
            std::string fileName = renderer::Game::getInstance()->getAppConfig().getRendererConfig().getConfigParser().
                                                                  getDefaultFontPath();
